...
class AutomateSheets:
    ...
    def __init__(self):
        ...
        self.endpoint = "http://api.gupshup.io/sm/api/v1/template/msg"
    ...
    def notify(self, data, row, message_type, curr_date, curr_day, time_zone_time):
        ...
        link = data[self.mapping["class_link"]].split('\n')
        msg_string = ""
        if message_type == "reminder":
            msg_string = {'id':config("TEMPLATEID_REM"), 'params': [data[self.mapping["child_first_name"]], \
                data[self.mapping["course_name"]], curr_date, curr_day, time_zone_time, \
                    link[0], link[1], link[2]]}
        else:
            msg_string = {'id':config("TEMPLATEID_MESS"), 'params': [data[self.mapping["child_first_name"]], \
                data[self.mapping["course_name"]], curr_date, curr_day, time_zone_time, \
                link[0], link[1], link[2]]}
        ...
        send_data = {
            ...
            'template' : json.dumps(msg_string),
        ...
        if req.status_code == 200:
            ...
            if message_type == "reminder":
                ...
                rng_sent = "Class Schedule" + f"!U{row+1}"
            else:
                val = int(data[self.mapping["message_retry"]]) + 1
                rng_sent = "Class Schedule" + f"!R{row+1}"
            self.sheet.values().update(spreadsheetId=self.spreadsheet_id, range=rng_sent, \
